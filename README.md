...
> **Tip:** IMS credentials can be provided either as a JSON array (`OAUTH_CLIENT_SECRETS=["secret"]`, the format generated by `npm run sync-oauth-credentials`) or as the legacy scalar `OAUTH_CLIENT_SECRET=secret`. The runtime accepts both inputs and normalizes them automatically.
