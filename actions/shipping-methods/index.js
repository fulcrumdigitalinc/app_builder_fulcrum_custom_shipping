...
const errorOp = (title) =>
  addOp({
    ...
    cost: 0,
    additional_data: [
      { key: 'source', value: 'shipping-methods error' },
    ],
  });
...
// ---------- NEW SIMPLE FLAG: replaces “ultra tolerante” ----------
function flag(v) {
  const s = String(v ?? '').trim().toLowerCase();
...
function extractStore(req = {}) {
  const cands = [
    ...
    req?.store_id,
    req?.quote?.store_id,
    req?.extension_attributes?.store_id,
    req?.address?.store_id,
  ];
...
function extractCustomerGroup(req = {}) {
  const cands = [
    req?.customer_group_id,
    req?.customer_group,
    req?.customerGroupId,
    req?.customer?.group_id,
    req?.quote?.customer_group_id,
...
// Store filter (STRICT from JSON): empty list ⇒ DO NOT show
function storesMatch(request, storesFromJson) {
  const allow = toArray(storesFromJson)
    .map(canonStoreToken)
    .filter(Boolean);
  if (!allow.length) return false;

  const reqStore = extractStore(request);
  if (reqStore == null) return true;
...
function extractCartTotal(req = {}) {
  const cands = [
    req?.totals?.grand_total,
    req?.totals?.base_grand_total,
    req?.grand_total,
    req?.base_grand_total,
    req?.package_value_with_discount,
    req?.packageValueWithDiscount,
    req?.package_value,
    req?.packageValue,
    req?.subtotal_incl_tax,
    req?.base_subtotal_incl_tax,
    req?.totals?.subtotal_with_discount,
    req?.totals?.base_subtotal_with_discount,
    req?.subtotal_with_discount,
    req?.base_subtotal_with_discount,
    req?.subtotal,
    req?.base_subtotal,
  ];
  ...
  for (const arr of itemArrays) {
    for (const it of arr) {
      ...
      const rowCandidates = [
        it?.row_total_with_discount,
        it?.base_row_total_with_discount,
        it?.row_total_incl_tax,
        it?.base_row_total_incl_tax,
        it?.row_total,
        it?.base_row_total,
      ];

      let row = rowCandidates
        .map(Number)
...
async function main(params) {
  ...
  try {
    ...
    if (!success) {
      return ok([errorOp(`Webhook verify: ${error}`)]);
    }

    const payload = JSON.parse(
      b64decode(params.__ow_body || 'e30='),
    );
    ...
    logger.info(
      `cartTotal = ${cartTotal}; cartItemCount = ${cartItemCount}`,
    );

    const DEFAULT_PRICE =
      Number(params.DEFAULT_PRICE ?? 0) || 0;
    ...
    if (!rawBase) {
      return ok([errorOp('Missing COMMERCE_BASE_URL')]);
    }

    const COMMERCE_BASE_URL = normalizeBaseUrl(rawBase);
    params.COMMERCE_BASE_URL = COMMERCE_BASE_URL;
    ...
    try {
      ...
      logger.warn(
        `aio-lib-files init failed: ${e.message}`,
      );
    }

    // ----------- REST CALL via Shared Client -----------
    let carriers = [];
    try {
      const commerceClient =
        await getAdobeCommerceClient(params);
      ...
      const {
        ...
        statusCode,
        body,
      } = await commerceClient.getOopeShippingCarriers();

      if (!carriersOk || !Array.isArray(message)) {
        const snippet = JSON.stringify(body || message || '')
          .replace(/\s+/g, ' ')
          .slice(0, 160);
        ...
        return ok([
          errorOp(
            `REST HTTP ${
              ...
              snippet ||
              'Error fetching oope_shipping_carrier'
            }`,
          ),
        ]);
      }
      ...
      return ok([
        errorOp(`REST fetch error: ${e.message}`),
      ]);
    }
    ...
    for (const c of carriers) {
      if (!c) continue;
      if (!isActiveFromRest(c.active)) continue;

      const fallbackTitle = String(
        c.method_name ?? c.title ?? 'Shipping Method',
      );

      const codeKey = String(
        c.code ?? slugify(fallbackTitle, 'custom'),
      );

      let custom = {};
      if (files) {
        try {
          custom = await readCustomFromFiles(
            files,
            codeKey,
            logger,
          );
          ...
          logger.warn(
            `readCustomFromFiles failed for ${codeKey}: ${e.message}`,
          );
        }
      }
      ...
      if (!groupsMatch(request, custom.customer_groups))
        continue;
      ...
      const unitPrice =
        pickNumber(custom.price, custom.value) !== null
          ? pickNumber(custom.price, custom.value)
          : DEFAULT_PRICE;

      // NEW FLAG IN USE HERE
      const ppiRaw =
        custom.price_per_item !== undefined
          ? custom.price_per_item
          : custom.pricePerItem;

      const pricePerItem = flag(ppiRaw);

      const finalPrice = pricePerItem
        ? unitPrice * (cartItemCount || 0)
        : unitPrice;
          ...
          additional_data: [
            {
              key: 'source',
              value: files
                ? 'REST + Files (min/max + stores + groups)'
                : 'REST (min/max + stores + groups)',
            },
            ...
            },
            {
              key: 'min',
              value:
                min === null ? 'none' : String(min),
            },
            {
              key: 'max',
              value:
                max === null ? 'none' : String(max),
            },
            {
              key: 'req_store',
              value: String(
                extractStore(request) ?? 'unknown',
              ),
            },
            {
              key: 'cfg_stores',
              value: JSON.stringify(
                toArray(custom.stores),
              ),
            },
            {
              key: 'price_per_item',
    ...
    if (ops.length === 0) {
      return ok([
        errorOp('No matching carriers after filters'),
      ]);
    }
    ...
    return ok([
      errorOp(
        `Exception: ${String(e.message || e).slice(0, 140)}`,
      ),
    ]);
  }
}
