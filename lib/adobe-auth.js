...
const { allNonEmpty } = require('./params');
...
function getClientSecrets(params) {
  return params.OAUTH_CLIENT_SECRET ?? params.OAUTH_CLIENT_SECRETS;
}
...
async function getAdobeAccessToken(params) {
  const config = {
    ...
    client_secrets: parseArrayEnv(getClientSecrets(params)),
    ...
    scopes: parseArrayEnv(params.OAUTH_SCOPES),
...
async function resolveAuthOptions(params) {
  const clientSecret = getClientSecrets(params);
  ...
  if (
    !!clientSecret &&
    allNonEmpty(
      { ...params, OAUTH_CLIENT_SECRET: clientSecret },
      [
      'OAUTH_CLIENT_ID',
      'OAUTH_CLIENT_SECRET',
  ...
  throw new Error(
    "Can't resolve authentication options for the given params. " +
      'Please provide either IMS options (OAUTH_CLIENT_ID, OAUTH_CLIENT_SECRET, OAUTH_TECHNICAL_ACCOUNT_ID, OAUTH_TECHNICAL_ACCOUNT_EMAIL, OAUTH_IMS_ORG_ID, OAUTH_SCOPES) ' +
